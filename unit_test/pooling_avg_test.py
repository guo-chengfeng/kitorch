from ..functional.pooling import avgpool2d
...
def main_pooling(inputs,kernel_size,padding=None,elapsed_time=False):
    ...
    out = avgpool2d(inputs,kernel_size,padding=padding)
    out_grad = rand_like(out)
    out.backward(out_grad)

    if elapsed_time:
        show_time("Numpy avgpool2d elapsed time:",(time.time()-start))

    t_inputs = torch.from_numpy(inputs.numpy())
    t_out_grad = torch.from_numpy(out_grad.numpy())
    t_inputs.requires_grad = True
    if padding:
        AvgPool2d = torch.nn.AvgPool2d(kernel_size,padding=padding)
    else:
        AvgPool2d = torch.nn.AvgPool2d(kernel_size)

    start = time.time()
    t_out = AvgPool2d(t_inputs)
    t_out.backward(t_out_grad)

    if elapsed_time:
        show_time("Torch avgpool2d elapsed time:",(time.time()-start))
